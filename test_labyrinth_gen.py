...
def test_single_cell():
    ...
    assert(labyrinth["start"] == (0,0))
    assert(labyrinth["ends"] == [])
...
def test_two_columns_by_one_row():
    ...
    assert(labyrinth["start"] in [(0, 0), (0, 1)]) # row/column coordinates!
    assert(labyrinth["ends"] == [(0, 0), (0, 1)])
...
def test_one_column_by_two_rows():
    ...
    assert(labyrinth["start"] in [(0, 0), (1, 0)])
    assert(labyrinth["ends"] == [(0, 0), (1, 0)])
    # open south wall
    # |
    # open north wall
